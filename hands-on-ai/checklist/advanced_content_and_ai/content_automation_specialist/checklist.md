# Compliance Checklist: CONTENT_AUTOMATION_SPECIALIST
Domain: ADVANCED_CONTENT_AND_AI | Type: AUDIT & VERIFICATION | Status: MANDATORY
================================================================================

## 1. Executive Summary & Audit Scope
AI is the engine of efficiency. This department pioneers 'Human-in-the-loop' (HITL) automation.
This document serves as the absolute source of truth for auditing the performance and compliance of the content_automation_specialist. All items must be verified.
...
- [ ] **CRITICAL**: Prompt Engineering must follow the 'Context-Task-Constraint' (CTC) framework.
      *Verification*: Check codebase/process to ensure strict adherence. No exceptions.
- [ ] **CRITICAL**: All AI-generated output must undergo automated bias and hallucination auditing.
      *Verification*: Check codebase/process to ensure strict adherence. No exceptions.
- [ ] **CRITICAL**: Model selection is based on Token-Cost-Latency optimization (TCLo).
      *Verification*: Check codebase/process to ensure strict adherence. No exceptions.
- [ ] **CRITICAL**: Multi-agent orchestration requires state-machine validation to prevent infinite loops.
      *Verification*: Check codebase/process to ensure strict adherence. No exceptions.
- [ ] **CRITICAL**: AI Ethics: All synthetic content must be watermarked per agency policy.
      *Verification*: Check codebase/process to ensure strict adherence. No exceptions.
- [ ] **CRITICAL**: Vector databases must be indexed using HNSW for sub-millisecond retrieval.
      *Verification*: Check codebase/process to ensure strict adherence. No exceptions.
- [ ] **CRITICAL**: Fine-tuning datasets must be sanitized for PII and copyright compliance.
      *Verification*: Check codebase/process to ensure strict adherence. No exceptions.
...
## 8. Final Sign-Off
By signing below, the auditor certifies that the content_automation_specialist implementation meets the agency's strict standards.
